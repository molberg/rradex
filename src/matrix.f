...
c     Set up rate matrix
      ...
      integer indx(maxlev)     ! needed for NumRep equation solver
      real*8  dsign            ! needed for NumRep equation solver
...
c      if ((niter.eq.1).and.(nfat.gt.0)) print*
c     $     ,"*** Warning: Some lines have very high optical depth"

c      if (debug) then
c         print*,yrate(1,1),yrate(1,2),yrate(1,3),yrate(1,4)
c         print*,yrate(2,1),yrate(2,2),yrate(2,3),yrate(2,4)
c         print*,yrate(3,1),yrate(3,2),yrate(3,3),yrate(3,4)
c         print*,yrate(4,1),yrate(4,2),yrate(4,3),yrate(4,4)
c      endif
...
c      if (debug) print*,'reducing matrix...' 
...
c      if (debug) print*,'nreduce=',nreduce
...
c         if (debug) print*,'inverting reduced matrix...'
c        call ludcmp(uarray,nreduce+1,maxlev,indx,dsign)
c        call lubksb(uarray,nreduce+1,maxlev,indx,rhs)
         call DGESV(nreduce+1,1,uarray,maxlev,indx,rhs,nreduce+1,ok)

c         if (debug) print*,'computing cascade...'
...
c         if (debug) print*,'inverting non-reduced matrix...'
c        call ludcmp(yrate,nplus,maxlev,indx,dsign)
c        call lubksb(yrate,nplus,maxlev,indx,rhs)
         call DGESV(nplus,1,yrate,maxlev,indx,rhs,nplus,ok)
      endif
...
C     Level populations are the normalized RHS components
      ...
      enddo
c      if (debug) print*,'total rhs=',total
c      if (debug) print*,'rhs=',(rhs(ilev),ilev=1,nlev)
...
c      if (debug) print*,'computing T_ex...'
...
c      if (debug) print*,niter,nthick,tsum,tsum/nthick,conv
      endif
...
C     ApJ 199, 69 (again with power law approximations)
        if(abs(3.0*tau).lt.0.1) then
         ...
         call rexit('Error: Escape probability method undefined')
      endif
      escprob = beta
      return
      end
