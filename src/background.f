...
C     option 3:  user-specified radiation field with spline interpolation
 21      format(A,$)
c         write(*,21) 'File with observed background intensity? '
         read *, bgfile
...
c         added 24aug2011 (thanks Simon Bruderer)
       ...
       if ((xnumin.lt.xnubr(1)).or.(xnumax.gt.xnubr(nrad))) 
     $ call rwarn(
     $ 'the line list requires extrapolation of the background')
...
C        enddo
      ...
      return
 666  call rexit('Error opening background file')
      end
...
c      write(*,202) xnu(iline)
          call rwarn('outside the range of the fitting function'
     $             //' and beyond lyman limit')
          call dblepr('xnu =', -1, xnu(iline), 1)
c  202 FORMAT(' ** XNU = ',1PE13.6,' IS OUTSIDE THE RANGE OF THE FITTING       
c     ; FUNCTION AND BEYOND LYMAN LIMIT')
        cbi=0.0d0
        cmi=0.0d0
...
1     if (khi-klo.gt.1) then
      ...
      h=xin(khi)-xin(klo)
      if (h.eq.0.d0) call rwarn('bad xin input in splintrp')
      a=(xin(khi)-x)/h
      b=(x-xin(klo))/h
