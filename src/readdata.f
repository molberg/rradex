...
c     in the header, every second line is a comment
  101 format(a)
      ...
      if (nlev.lt.1) call rexit('too few energy levels defined')
      if (nlev.gt.maxlev) call rexit('too many energy levels defined')
...
c     Term energies and statistical weights
     ...
     $       call rexit('illegal level number')
      enddo
...
c     Radiative upper & lower levels and Einstein coefficients
      ...
      if (nline.lt.1) 
     $    call rexit('error: too few spectral lines defined')
      if (nline.gt.maxline) 
     $    call rexit('error: too many spectral lines defined')

      do iline=1,nline
         read(11,*) dummy,iupp(iline),ilow(iline),aeinst(iline)
     $      ,spfreq(iline),eup(iline)
         if ((dummy.lt.1).or.(dummy.gt.nline))
     $        call rexit('error:illegal line number')
         xnu(iline)=(eterm(iupp(iline))-eterm(ilow(iline)))
         if ((xnu(iline).lt.eps))
     $        call rexit('illegal line frequency')
      enddo

c     Number of collision partners
      read(11,*)
      read(11,*) npart
      if (npart.lt.1) call rexit('too few collision partners defined' )
      if (npart.gt.maxpart) call rexit('too many collision partners')
 ...
 102  format(i1,a)
      do ipart=1,npart
         ...
         read(11,*)
         read(11,*) ncoll
      if (ncoll.lt.1) call rexit('too few collision rates defined' )
      if (ncoll.gt.maxcoll) call rexit('too many collision rates')
         read(11,*)
         read(11,*) ntemp
         if (ntemp.lt.0) 
     $       call rexit('too few collision temperatures defined')
         if (ntemp.gt.maxtemp) 
     $       call rexit('too many collision temperatures')
         read(11,*)
     ...
     $        call rexit('illegal collision number')
         enddo
...
C===  Tkin too high :
                  if (tkin.ne.temp(ntemp)) then
                     call rwarn('Tkin higher than temperatures '/
...
C     Tkin too low :
               if (tkin.ne.temp(1)) then
                  call rwarn('Tkin lower than temperatures '//
     %                 'for which rates are present.')
               endif
               do icoll=1,ncoll
                  iup=lcu(icoll)
...
c     Quite a big IF:
     ...
     $     .and.(density(3).lt.eps)) then
         ...
         call rwarn('Assuming thermal o/p ratio for H2')
         call dblepr('opr =', -1, opr, 1)
      endif
...
c     warning is given without reason. May fix this later if needed.
     ...
     $                    density(ipart)*colld(jpart,iup,ilo)
      ...
      if (.not.found) then
         call rexit('No rates found for any collision partner')
      endif
...
c     Calculate total collision rates (inverse collisional lifetime)
      ...
      return
   99 call rexit('error opening data file ' // molfile)
      end
